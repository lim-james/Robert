...
1. The team leader should fork this project into their own repository
2. Add your teammates as collaborators
3. Read and understand the code
4. The team leader should add this repository as an upstream remote repository if you want to get the latest bug fix or improvements. [Configuring a remote for a fork](https://help.github.com/articles/configuring-a-remote-for-a-fork)
5. Periodically sync changes to this repository to the get latest awesome code. [Syncing a fork](https://help.github.com/articles/syncing-a-fork)
...
##### update(double dt)
This is the meat of your code, the game logic should be done here. i.e. a reaction to input, enemy movement, enemy spawning, win game condition, lose game condition, state of the game, etc.
Sounds can be played here.

##### render()
Renders the current state of the game onto the screen. Go to a specific coordinate on the screen, set colours, write output, etc.
...
These 6 functions write to the buffer at that coordinate, you can use C-Style strings, C++ string class or a char. The attribute is a optional parameter. The last 3 functions are overloaded versions of the first 3 functions.
